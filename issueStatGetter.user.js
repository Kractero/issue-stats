...
// @downloadURL  https://github.com/Kractero/issue-stats/raw/main/tampermonkeyscript.js
// @updateURL    https://github.com/Kractero/issue-stats/raw/main/tampermonkeyscript.js
// @grant        none
// ==/UserScript==
...
async function getBadgeMap(nation) {
  ...
  const ranks = await fetch(
    `https://www.nationstates.net/nation=${nation}/detail=rank&userclick=${Date.now()}`,
    {
      headers: {
        'User-Agent': `Issue Stat Getter (by:Kractero, usedBy:{${nation}})`,
      },
    }
  );
  ...
  const categories = document.querySelectorAll(
    '#trophyranks tbody tr td:first-child'
  );
...
(async function () {
  ...
  const shownValues = JSON.parse(
    localStorage.getItem('uncheckedValues') ?? '{}'
  );
  ...
  function figureOutCheckStatus() {
    ...
    const uncheckedCheckboxes = Array.from(checkboxers).filter(
      (cb) => cb.checked
    );
    const uncheckedValues = Array.from(uncheckedCheckboxes, (cb) => cb.name);

    let currUncheckedObj =
      JSON.parse(localStorage.getItem('uncheckedValues')) || {};
    currUncheckedObj[nation] = uncheckedValues;

    localStorage.setItem('uncheckedValues', JSON.stringify(currUncheckedObj));

    Array.from(filterSpot.querySelectorAll('.effect-selector')).forEach(
      (effect) => {
        effect.classList.toggle(
          'effect',
          currUncheckedObj[nation].some((value) =>
            value === 'Health'
              ? !effect.textContent.includes('Public') &&
                effect.textContent.includes(value)
              : value === 'Taxation'
              ? !effect.textContent.includes('Freedom From') &&
                effect.textContent.includes(value)
              : value === 'Average Income'
              ? !effect.textContent.includes('of Poor') &&
                !effect.textContent.includes('of Rich') &&
                effect.textContent.includes(value)
              : effect.textContent.includes(value)
          )
        );
  ...
  toggleCheckStatus.addEventListener('click', () => {
    ...
    const areAllChecked = Array.from(checkboxList).every(
  ...
  toggleOnePercent.addEventListener('click', () =>
    toggleBadgeCheckboxes('-1.png')
  );
  toggleFivePercent.addEventListener('click', () =>
    toggleBadgeCheckboxes('-5.png')
  );
  toggleTenPercent.addEventListener('click', () =>
    toggleBadgeCheckboxes('-10.png')
  );
  toggleUnranked.addEventListener('click', () =>
    toggleBadgeCheckboxes('-100.png')
  );
  ...
  const choicesObject = Array.from(choices).map((choice) =>
  ...
  const getSubList = await fetch(
    `https://raw.githubusercontent.com/Kractero/issue-stats/master/json/${range}.json`
  );
  const issueSubList = await getSubList.json();
  const correspondIdx = Object.keys(issueSubList).filter(
    (issue) => issue === id
  );
  const issue = issueSubList[Number(correspondIdx)];
  const selections = Object.keys(issue.options).map(
    (choice) => issue.options[choice].text
  );
  ...
  function toggleBadgeCheckboxes(suffix) {
    let badges = JSON.parse(localStorage.getItem('badges') || '{}');
    if (badges[nation]) {
      const matchingKeys = Object.keys(badges[nation]).filter((key) =>
        badges[nation][key].endsWith(suffix)
      );
      const checkboxList = checkboxes.querySelectorAll(
        'input[type="checkbox"]'
      );
      checkboxList.forEach((checkbox) => {
        ...
        if (
          matchingKeys.includes(checkboxId) ||
          matchingKeys.includes(checkboxName)
        ) {
      ...
      return split
        .map((effect) => {
          const matchedEntry = Object.entries(badges[nation]).find(([key]) => {
            return key === 'Health'
              ? !effect.includes('Public') && effect.includes(key)
              : key === 'Taxation'
              ? !effect.includes('Freedom From') && effect.includes(key)
              : key === 'Average Income'
              ? !effect.includes('of Poor') &&
                !effect.includes('of Rich') &&
                effect.includes(key)
              : effect.includes(key);
          });
  ...
  approves.forEach((option, index) => {
    ...
    if (json[index]) {
      json[index].forEach((item) => {
        ...
        const meanValue = effect.includes('mean')
          ? effect.split('mean')[1].trim()
          : effect.split(' ')[0];
        ...
        isNegative
          ? badDiv.appendChild(effectDiv)
          : goodDiv.appendChild(effectDiv);

        if (showValues && JSON.parse(showValues)[nation]) {
          const shownValues = JSON.parse(showValues)[nation];
          effectDiv.classList.toggle(
            'effect',
            shownValues.some((value) =>
              value === 'Health'
                ? !effect.includes('Public') && effect.includes(value)
                : value === 'Taxation'
                ? !effect.includes('Freedom From') && effect.includes(value)
                : value === 'Average Income'
                ? !effect.includes('of Poor') &&
                  !effect.includes('of Rich') &&
                  effect.includes(value)
                : effect.includes(value)
            )
          );
        }
